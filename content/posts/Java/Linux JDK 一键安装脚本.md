---
title: "Linux JDK一键安装脚本"
description: "Linux JDK一键安装脚本"
keywords: "Java"
...
java_dist="/root/jdk-8u202-linux-x64.tar.gz"
default_java_dir="/usr/local"
...
function usage() {
    ...
    echo ""
    echo "-f: The jdk tar.gz file. 默认安装的是/root/jdk-8u202-linux-x64.tar.gz"
    echo "-p: java默认安装在/usr/local目录中，你可以通过-p命令切换目录"
    echo "-h: 显示帮助."
    echo ""
}
...
if ! [ -x "$(command -v axel)" ]; then
  wget https://mirrors.huaweicloud.com/java/jdk/8u202-b08/jdk-8u202-linux-x64.tar.gz -O /root/jdk-8u202-linux-x64.tar.gz

else
  axel -n 10 -a https://mirrors.huaweicloud.com/java/jdk/8u202-b08/jdk-8u202-linux-x64.tar.gz -o /root/jdk-8u202-linux-x64.tar.gz
...
```shell
chmod +x install-java.sh && ./install-java.sh
```
